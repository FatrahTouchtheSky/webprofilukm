    ...
    <div class="container">
        ...
        <main class="main-content">
            ...
            <div class="search-add">
                ...
                <button class="add-new" onclick="window.location.href='{{ url('/admin/addnews') }}'">
                ...
                <table class="gallery-table">
                    ...
                    <tbody id="tableBody">
                        @foreach ($newss as $news)
                            @php
                                Log::info("Status of news {$news->id}: {$news->status}");
                            @endphp
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $news->title }}</td>
                                <td>{{ Str::limit(preg_replace('/\s+/', ' ', $news->body), 40) }}</td>
                                <td>{{ $news->created_at->format('d M Y') }}</td>
                                <td><img src="{{ asset('img/' . $news->image) }}" alt="Image {{ $loop->iteration }}"
                                        style="width:100px; height:auto;"></td>
                                <td class="button-container">
                                    <button class="button"
                                        onclick="window.location.href='{{ route('news.edit', $news->id) }}'">
